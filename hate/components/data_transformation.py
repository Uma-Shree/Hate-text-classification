...
class DataTransformation:
    ...
    def concat_dataframe(self):
        try:
            logging.info("Entered into concat_dataframe method of DataTransformation class")
    ...
    def concat_data_cleaning(self, words):
        try:
            ...
            words = re.sub(r'\[.*?\]', '', words)  # Raw string
            words = re.sub(r'https?://\S+|www\.\S+', '', words)  # Raw string
            words = re.sub('<.*?>+', '', words)
            words = re.sub('[%s]' % re.escape(string.punctuation), '', words)
            words = re.sub(r'\n', '', words)  # Raw string
            words = re.sub(r'\w*\d\w*', '', words)  # Raw string
            words = [word for word in words.split(' ') if word not in stopword]  # Fixed: 'word' not 'words'
            words=" ".join(words)
